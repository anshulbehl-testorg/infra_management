Example_corp.Infra_management Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: example_corp.infra_management.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from example_corp.infra_management
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: example_corp.infra_management.run
      vars:
        run_x: 42
```
